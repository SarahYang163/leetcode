...
def choose_word():
    # 查出所有的记录
    results = engine.connect().exec_driver_sql("select English,Chinese,score,id from sentence  ")
    # 将记录格式为字典
...
def display():
    ...
    sql1 = "update  sentence set score ={}-1 where id ={}".format(chosen_word["score"], chosen_word["id"])
    sql0 = "update  sentence set score ={}+1 where id ={}".format(chosen_word["score"], chosen_word["id"])
    try:
