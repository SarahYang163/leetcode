...
def choose_word():
    # 查出所有的记录
    results = engine.connect().exec_driver_sql("select English,Chinese,score,id from English  ")
    # 将记录格式为字典
    ...
    response = (requests.get("https://dict.iciba.com/dictionary/word/suggestion",params={"word": chosen_word["English"]})).json()["message"]
    if response:
...
def display():
    ...
    sql1 = "update  English set score ={}-1 where id ={}".format(chosen_word["score"], chosen_word["id"])
    sql0 = "update  English set score ={}+1 where id ={}".format(chosen_word["score"], chosen_word["id"])
    try:
